...
// v1.46 - IMplement MQTT counter reset option from adafruit  
...
void handleRoot() {
  ...
  String commandFeedPath = IO_USERNAME + String("/feeds/") + deviceID + "-command";
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (message.startsWith("update:")) {
    ...
    return; // Don't fall through to "unknown command"
  }
...
void setup() {
  ...
  commandFeedPath = String(IO_USERNAME) + "/feeds/" + deviceID + "-command";
